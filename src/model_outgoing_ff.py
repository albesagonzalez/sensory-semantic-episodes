...
class SSCNetwork(nn.Module):
    ...
    def mtl_generate(self, semantic_load, num_iterations=None):
        ...
        if getattr(self, "sensory_replay_only", False):
            ...
            h_sensory = self.pattern_complete(
                ...
                sparsity=mtl_sensory_sparsity,
            )
            h = torch.zeros(self.mtl_size)
            h[:self.mtl_sensory_size] = h_sensory
            return h
    ...
    def hebbian(self, post_region, pre_region, sleep=False, quick=False, inh=False):
        if self.frozen:
          ...
          if w_name in {'ctx_mtl', 'mtl_semantic_ctx'}:
            IM = getattr(self, pre_region + '_IM')
            IM_lmbda = getattr(self, 'max_pre_' + w_name)/ torch.sum(getattr(self, post_region))
    ...
    def homeostasis(self, post_region, pre_region, quick=False, inh=False):
        if self.frozen:
          ...
          if w_name in {"ctx_mtl", "mtl_semantic_ctx"}:
            setattr(self, pre_region + '_IM', (torch.sum(w, dim=0).round() < max_pre_connectivity).float())
          setattr(self, w_name, w)
    ...
    def init_network(self, net_params):
      ...
      self.frozen = False
      self.lesioned = {}
      self.sensory_replay_only = False
      ...
      self.mtl_IM = torch.ones((self.mtl_size))
